...
const SavedStartUpCard: FC<Props> = ({ startup }) => {
  ...
  const onUnSaveStartUpHandler = async () => {
    ...
    try {
      if (docId) {
        ...
        await queryClient.refetchQueries({ queryKey: ["savedStartUpsData"] });
  ...
  return (
    <div className="flex flex-col justify-between items-center gap-1 sm:gap-2 md:gap-3  bg-[#f6f9fd] rounded-[5px] relative ">
      <Link href={startup?.slug?.name !== undefined ? `/startup/${startup?.slug?.name}` : "#"}>
        <div className="">
      ...
      <div className="w-full  px-3  mb-3">
        {isFollowed && isFollowed?.status === "pending" ? (
          <div
            onClick={async () => await onUnfollowHandler(startup)}
            className=" w-full flex   justify-center  items-center gap-1 sm:gap-2 md:gap-3 rounded-full px-1 sm:px-4 md:px-8 lg:px-12 py-1 sm:py-2 md:py-3 border border-black cursor-pointer "
          >
            {/* <div className=" ">
          ...
          <div
            onClick={async () => await onFollowHandler(startup)}
            className=" w-full flex  justify-center  items-center gap-1 sm:gap-2 md:gap-3 rounded-full px-1 sm:px-4 md:px-8 lg:px-12 py-1 sm:py-2 md:py-3 border border-primary cursor-pointer "
          >
            <div className=" ">
      ...
      <div
        ...
        className="absolute  top-0 right-0 transform -translate-x-1/2 translate-y-1/2  cursor-pointer "
      >
        <FlatIcon className="flaticon-close  md:text-xl sm:text-lg text-base text-black" />
      </div>
    </div>
...
export default SavedStartUpCard;
