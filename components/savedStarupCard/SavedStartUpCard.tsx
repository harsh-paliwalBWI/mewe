    ...
    const onUnSaveStartUpHandler = async () => {
        ...
        try {
          if (docId) {
            ...
            const updatedSavedStartups = existingSavedStartups.filter((item: any) => item?.id !== startup?.id);
            await setDoc(docRef, { savedStartups: updatedSavedStartups }, { merge: true });
            await queryClient.invalidateQueries({ queryKey: ["savedStartUpsData"] });
...
{isFollowed&&isFollowed?.status==="pending" ? (
    ...
    {/* </div> */}
    <div onClick={async () => await onUnSaveStartUpHandler()} className="absolute  top-0 right-0 transform -translate-x-1/2 translate-y-1/2  cursor-pointer ">
      <FlatIcon className="flaticon-close  md:text-xl sm:text-lg text-base text-black" />
    </div>
  </div>
  )
